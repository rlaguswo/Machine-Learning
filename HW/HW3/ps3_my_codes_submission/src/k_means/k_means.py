...
def init_centroids(num_clusters, image):
    ...
    shape = (num_clusters, np.shape(image)[-1])
    centroids_init = np.zeros(shape)

    for i in range(num_clusters):
        h = random.randint(0, np.shape(image)[0]-1)
        w = random.randint(0, np.shape(image)[1]-1)
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    W = np.shape(image)[1]
    C = np.shape(image)[2]
    shape = (num_clusters, C)
    new_centroids = np.zeros(shape)
    D_shape = (num_clusters, 1)
    for i in range(max_iter):
        if i%print_every == 0:
            print('[INFO] Status Update {}'.format(i))
        D = np.zeros(D_shape)
        x = np.zeros(shape)
        for h in range(H):
            for w in range(W):
                C_list = np.zeros(D_shape)
                for j in range(num_clusters):
                    c = image[h,w,:] - centroids[j,:]
                    l1_square = c.T@c
                    C_list[j] = l1_square
                
                c_i = np.argmin(C_list)
                
                x[c_i] = x[c_i] + image[h,w,:]
                D[c_i] = D[c_i] + 1

        for k in range(num_clusters):
            if D[k] != 0:
                new_centroids[k,:] = x[k,:]/D[k]
