...
def update_W(W, x, learning_rate):
   ...
   #logistic
    #sigmoid = 1 / (1 + np.exp(-W.dot(x[:,None])))
    #s = 1-2*sigmoid
    #updated_W = W + learning_rate*(np.linalg.inv(W.T) + s @ x[:,None].T)
    
    #laplace
    #x[:,None] from: https://stackoverflow.com/questions/33350147/how-to-convert-5-numpy-array-to-5-1
    s = np.sign(W@x[:,None])
    updated_W = W + learning_rate*(np.linalg.inv(W.T) - s @ x[:,None].T)
    # *** END CODE HERE ***
    return updated_W
