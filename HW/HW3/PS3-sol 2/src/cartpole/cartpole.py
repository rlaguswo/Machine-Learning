...
def choose_action(state, mdp_data):
    ...
    score1 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
    score2 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        new_value = np.zeros(mdp_data['num_states'])

        iterations = iterations + 1
        for s in range(mdp_data['num_states']):
            value1 = mdp_data['transition_probs'][s, :, 0].dot(mdp_data['value'])
            value2 = mdp_data['transition_probs'][s, :, 1].dot(mdp_data['value'])

            new_value[s] = max(value1, value2)

        new_value = mdp_data['reward'] + gamma * new_value

        max_diff = float('-Inf')
        for s in range(mdp_data['num_states']):
            if abs(new_value[s] - mdp_data['value'][s]) > max_diff:
                max_diff = abs(new_value[s] - mdp_data['value'][s])
