...
def choose_action(state, mdp_data):
    ...
    #[state, :, 0] from: https://www.pythonlikeyoumeanit.com/Module3_IntroducingNumpy/AccessingDataAlongMultipleDimensions.html
    choice_0 = transition_probs[state, :, 0] @ value
    choice_1 = transition_probs[state, :, 1] @ value
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # Number of times we took action a in state s
    D = np.sum(transition_counts, axis = 1)

    for i in range(num_states):
        if D[i, 0] != 0:
            transition_probs[i,:, 0] = transition_counts[i, :, 0] / D[i, 0]

        if D[i][1] != 0:
            transition_probs[i,:,1] = transition_counts[i,:,1] / D[i, 1]

        if reward_counts[i, 1] != 0:
            reward[i] = -reward_counts[i, 0] / reward_counts[i, 1]
    
    mdp_data['transition_data'] = transition_probs
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    #Bellman's equation:
    #Value iteration
    # V*(s) = R(s) + max_a(gamma(sum P_sa(s')V*(s')))
    ...
    while True:
        ...
        for i in range(num_states):
            action_0 = gamma*transition_probs[i, :, 0] @ mdp_data['value']
            action_1 = gamma*transition_probs[i, :, 1] @ mdp_data['value']

            V_s[i] = max(action_0, action_1)

        V_s = reward +  V_s

        change = np.absolute(V_s - mdp_data['value'])
        max_abs_change = change[0]
        
        mdp_data['value']= V_s
    
        for i in range(num_states):
            if change[i]> max_abs_change:
                max_abs_change = change[i]
