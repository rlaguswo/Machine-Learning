...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    #return sign((theta).T*phi(X))
    beta_list, x_list = state
    sum = 0

    #beta_list and x_list has same length. 
    for i in range(len(beta_list)):
        a = beta_list[i]
        b = x_list[i]
        sum = sum + a * kernel(b, x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    new_beta = learning_rate * (y_i - prediction)
