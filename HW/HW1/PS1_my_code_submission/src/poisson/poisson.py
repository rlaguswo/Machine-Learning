...
def main(lr, train_path, eval_path, save_path):
    ...
    plt.scatter(y_eval, prediction, c ="blue")
    plt.xlabel("True Count")
    plt.ylabel("Corresponding Predicted Expected Count")
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        #theta startes with zero vector
        ...
        for i in  range(self.max_iter):
            ...
            gradient = x.T @ (y - np.exp(x.dot(self.theta)))
            
            #updtae theta
            self.theta = self.theta + self.step_size * gradient

            #Measure the change in parameters
            a = self.theta - theta_kth
            dst = np.linalg.norm(a)
    ...
    def predict(self, x):
        ...
        """
        # *** START CODE HERE ***
        # put the x-input to the sigmoid. 
        
        prediction = np.exp(x.dot(self.theta))
        return prediction
        # *** END CODE HERE ***
